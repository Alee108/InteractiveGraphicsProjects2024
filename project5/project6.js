...
float checkShadow(vec3 position, vec3 lightPosition){
	Ray ray;
	ray.dir = normalize(lightPosition - position);
...
bool IntersectRay( inout HitInfo hit, Ray ray )
{
	...
	for ( int i=0; i<NUM_SPHERES; ++i ) {
		...
		vec3 f = ray.pos - spheres[i].center; // this is the term f in the equation (d*d)t^2 + 2*(f*d)t + f*f-r^2
		float a = dot(ray.dir, ray.dir);
		float b = 2.0 * dot(f, ray.dir);
		float c = (dot(f,f)- pow(spheres[i].radius, 2.0));
